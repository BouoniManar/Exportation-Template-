// src/components/layout/Sidebar.tsx
import React from 'react';
import { NavLink, Link, useNavigate } from 'react-router-dom';
import {
  FaTachometerAlt,
  FaUsersCog,
  FaShieldAlt,
  FaSignOutAlt,
  // FaUserCircle, // Moins pertinent si on affiche toujours une image
  FaCog,
  FaUserEdit
} from 'react-icons/fa';
import { IconType } from 'react-icons';
import { useAuth } from '../../context/AuthContext'; // Assurez-vous que le chemin est correct

// MODIFICATION 1: Importer l'image locale
// Ajustez le chemin si la structure de vos dossiers est différente
// Ce chemin suppose que Sidebar.tsx est dans src/components/layout/
// et l'image dans src/assets/images/
import adminDefaultAvatar from '../../assets/images/admin.jpg';// <-- IMPORTATION DE L'IMAGE

interface NavLinkItem {
  name: string;
  icon: IconType;
  path: string;
}

const adminNavigationLinks: NavLinkItem[] = [
  { name: 'Dashboard Admin', icon: FaTachometerAlt, path: '/admin' },
  { name: 'Gérer Utilisateurs', icon: FaUsersCog, path: '/manage-users' },
  { name: 'Supervision Plateforme', icon: FaShieldAlt, path: '/admin/platform-supervision' },
  { name: 'Gérer Profil', icon: FaUserEdit, path: '/admin-profile' },
  { name: 'Paramètres', icon: FaCog, path: '/admin-settings' },
];

const API_BASE_URL = "http://127.0.0.1:8001"; // Ou votre variable d'environnement

const AdminSidebar = () => {
  const { user, logout, isLoading } = useAuth();
  const navigate = useNavigate();

  const handleLogout = () => {
    logout();
    navigate('/login');
  };

  const baseLinkClasses = "flex items-center px-4 py-2 text-gray-300 hover:bg-gray-700 hover:text-white rounded-md transition-colors duration-200 group";
  const activeLinkClasses = "bg-gray-700 text-white font-semibold";
  const iconBaseClasses = "mr-3 h-4 w-4 flex-shrink-0 text-gray-400 group-hover:text-gray-300 transition-colors duration-200";
  const iconActiveClasses = "text-white";

  const renderSidebarAvatar = () => {
    const avatarContainerClasses = "w-16 h-16 mb-2";
    // const avatarInitialFontSize = "text-xl"; // Moins pertinent si on utilise une image

    if (isLoading) {
      return <div className={`${avatarContainerClasses} rounded-full bg-gray-700 animate-pulse`}></div>;
    }

    let avatarSrc = adminDefaultAvatar; // Utiliser l'image locale par défaut
    let avatarAlt = "Avatar Administrateur";

    if (user && user.avatarUrl) {
      // Si l'utilisateur admin a une URL d'avatar spécifique, l'utiliser
      avatarSrc = user.avatarUrl.startsWith('http') || user.avatarUrl.startsWith('blob:')
        ? user.avatarUrl
        : `${API_BASE_URL}${user.avatarUrl}`;
      avatarAlt = `Avatar de ${user.name || 'Administrateur'}`;
    } else if (user && user.name && !user.avatarUrl) {
      // Optionnel: Si pas d'avatarUrl mais un nom, vous pourriez afficher les initiales comme avant
      // ou simplement utiliser l'avatar par défaut. Pour cet exemple, on utilise l'avatar par défaut.
      // Si vous préférez les initiales:
      // const initials = user.name.split(' ').map(n => n[0]).slice(0, 2).join('').toUpperCase();
      // return (
      //    <span className={`flex items-center justify-center ${avatarContainerClasses} rounded-full bg-blue-600 text-white ${avatarInitialFontSize} font-semibold border-2 border-gray-600`}>
      //       {initials}
      //    </span>
      //  );
    }
    // Pour tous les autres cas (pas d'utilisateur, pas d'avatarUrl spécifique), adminDefaultAvatar est déjà défini

    return (
      <img
        key={avatarSrc} // Utiliser avatarSrc comme clé car elle peut changer
        src={avatarSrc}
        alt={avatarAlt}
        onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => {
          // En cas d'erreur avec user.avatarUrl, afficher l'avatar local par défaut
          const target = e.target as HTMLImageElement;
          if (target.src !== adminDefaultAvatar) { // Éviter boucle si l'image par défaut elle-même échoue
            target.onerror = null;
            target.src = adminDefaultAvatar;
            target.alt = "Avatar Administrateur par défaut";
          }
        }}
        className={`${avatarContainerClasses} rounded-full object-cover border-2 border-gray-600 bg-gray-700`}
      />
    );
  };

  return (
    <div className="hidden md:flex md:flex-col md:w-64 bg-gray-800 text-white flex-shrink-0 h-full shadow-lg">
      <div className="flex items-center justify-center h-14 border-b border-gray-700 flex-shrink-0 px-3">
        <Link to="/admin" className="text-lg font-bold text-white hover:text-gray-200 truncate">
           Admin JsonToUI
        </Link>
      </div>

      <div className="p-3 mt-2 mb-1 flex flex-col items-center text-center border-b border-gray-700 pb-3 flex-shrink-0">
        {renderSidebarAvatar()}
        {isLoading ? (
          <p className="text-xs text-gray-400">Chargement...</p>
        ) : user ? (
          <>
            <p className="text-sm font-semibold text-gray-100 mt-1 truncate w-full" title={user.name || undefined}>
              {user.name || 'Admin connecté'}
            </p>
            <p className="text-xs text-gray-400 mt-0.5 truncate w-full" title={user.email || undefined}>
              {user.email || 'Aucun email'}
            </p>
          </>
        ) : (
          <p className="text-xs text-gray-400">Non connecté</p>
        )}
      </div>

      <div className="flex flex-col flex-1 overflow-y-auto">
        <nav className="flex-1 px-2 py-1.5 space-y-0.5">
          {adminNavigationLinks.map((link) => {
            const LinkIcon = link.icon;
            return (
              <NavLink
                key={link.name}
                to={link.path}
                className={({ isActive }) =>
                  `${baseLinkClasses} ${isActive ? activeLinkClasses : ''}`
                }
              >
                {({ isActive }) => (
                  <>
                    {LinkIcon && (
                      <LinkIcon
                        className={`${iconBaseClasses} ${isActive ? iconActiveClasses : ''}`}
                        aria-hidden="true"
                      />
                    )}
                    <span className="truncate text-sm">{link.name}</span>
                  </>
                )}
              </NavLink>
            );
          })}
        </nav>

        <div className="px-2 py-2.5 mt-auto border-t border-gray-700 flex-shrink-0">
           {(() => {
              if (isLoading || !user) {
                  return null;
              }
              const LogoutIcon = FaSignOutAlt;
              return (
                 <button
                    onClick={handleLogout}
                    className={`${baseLinkClasses} w-full text-left`}
                  >
                    {LogoutIcon && (
                      <LogoutIcon
                        className={`${iconBaseClasses}`}
                        aria-hidden="true"
                      />
                    )}
                    <span className="truncate text-sm">Déconnexion</span>
                  </button>
              );
           })()}
        </div>
      </div>
    </div>
  );
};

export default AdminSidebar;